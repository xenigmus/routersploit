...
def print_results(results):
    for ip, proc in results:
        ...
        if output is not None:
            vulnerabilities = re.findall(r"Could not verify exploitability:(.*?)\n\n", output, re.DOTALL)
            if vulnerabilities:
                vulnerabilities = vulnerabilities[0].strip()
                vulnerabilities = re.sub(r"(\n\s*){2,}", "\n", vulnerabilities)
                print(f"\n [*] Verified Vulnerabilities for {ip}:")
